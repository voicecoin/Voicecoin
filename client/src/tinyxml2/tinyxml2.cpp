...
#else
        char test[2] = {0};
        int len = vsnprintf( test, 0, format, va );
...
void XMLPrinter::Print( const char* str) {
    if ( _fp ) {
        fprintf( _fp, "%s", str);
    }
    char* p = _buffer.PushArr( strlen(str) ) - 1;
    strcat(p, str);
}
void XMLPrinter::Print( const char* format, char c) {
    if ( _fp ) {
        fprintf( _fp, format, c);
    }
    char* p = _buffer.PushArr( strlen(format) + 1 -2 ) - 1;
    sprintf(p, format, c);
}
void XMLPrinter::Print( const char* format, const char *str) {
    if ( _fp ) {
        fprintf( _fp, format, str);
    }
    char* p = _buffer.PushArr( strlen(format) + strlen(str) -2 ) - 1;
    sprintf(p, format, str);
}

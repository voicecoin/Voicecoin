...
function getblockinfo($dbconn, $emercoin, $hash) {
	file_put_contents('log.txt', "getblockinfo......\n",FILE_APPEND);
	...
	$current_id="";
	$hash=$block['hash'];
	file_put_contents('log.txt', $hash,FILE_APPEND);
	...
	file_put_contents('log.txt', "height=".$height,FILE_APPEND);
	file_put_contents('log.txt', "\n",FILE_APPEND);
	$time=$block['time'];
	$time=strtotime($time);
	$nonce=$block['nonce'];
	...
	if (isset($block['previousblockhash'])){
		file_put_contents('log.txt', "previousblockhash=".$block['previousblockhash'],FILE_APPEND);
		file_put_contents('log.txt', "\n",FILE_APPEND);
		...
		$previousblockhash="";
		file_put_contents('log.txt', "No previousblockhash\n",FILE_APPEND);
	}
	...
	$hash_in_db="";
	$query = "SELECT id, hash, status FROM blocks WHERE id = ".($height);
	$result = $dbconn->query($query);
	$dbconn->error;
	$current_status="";
		while($row = $result->fetch_assoc())
	...
	file_put_contents('log.txt', "hash_in_db=".$hash_in_db,FILE_APPEND);
	file_put_contents('log.txt', "\n",FILE_APPEND);
	file_put_contents('log.txt', "current_status=".$current_status,FILE_APPEND);
	file_put_contents('log.txt', "\n",FILE_APPEND);
	
	if ($hash_in_db == "" && $current_status!="0") {
		
		file_put_contents('log.txt', 'insert',FILE_APPEND);
		file_put_contents('log.txt', "\n",FILE_APPEND);
		...
		file_put_contents('log.txt', "INSERT INTO blocks\n",FILE_APPEND);
	...
	if ($new_ID!="") {
		...
		file_put_contents('log.txt', "new_ID=".$new_ID,FILE_APPEND);
		file_put_contents('log.txt', "\n",FILE_APPEND);
		...
		foreach ($block['tx'] as $txid) {
			...
			file_put_contents('log.txt', "txid=".$txid,FILE_APPEND);
			file_put_contents('log.txt', "\n",FILE_APPEND);
	...
	if ($update_ID!="") {
		...
		file_put_contents('log.txt', "update_ID=".$update_ID,FILE_APPEND);
		file_put_contents('log.txt', "\n",FILE_APPEND);
		...
		foreach ($block['tx'] as $txid) {
			...
			foreach ($vin['sentaddressarray'] as $address => $value) {
				...
				file_put_contents('log.txt', "sentaddressarray 1",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
				...
				$senttransactionaddress[$txid][$address]="1";
					...
					file_put_contents('log.txt', "sentaddressarray 2",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
			}
			$valuein=$vin["valuein"];
			$countvin=$vin["countvin"];
			...
			$avgcoindaysdestroyed=$vin["avgcoindaysdestroyed"];
				...
				file_put_contents('log.txt', "xxxx2",FILE_APPEND);
			file_put_contents('log.txt', "\n",FILE_APPEND);
			...
			foreach ($vout['receiveaddressarray'] as $address => $value) {
					...
					file_put_contents('log.txt', "receiveaddressarray 1",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
				if ($addressquery=="") {
					$addressquery="address='".$address."'";
				} else {
				...
				if (!isset($addresses[$address]['receivedvalue'])) {
					...
					$addresses[$address]['receivedcount']++;
						...
						file_put_contents('log.txt', "receiveaddressarray 2",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
			}
			
			file_put_contents('log.txt', "mmmm 1",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
			...
			file_put_contents('log.txt', "mmmm 2",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
				
			$result = $dbconn->query($query);
			
			file_put_contents('log.txt', "mmmm 3",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
				
			$valueintotal=$valueintotal+$valuein;
			$valueouttotal=$valueouttotal+$valueout;
			file_put_contents('log.txt', "mmmm 4",FILE_APPEND);
			file_put_contents('log.txt', "\n",FILE_APPEND);
			
			$feetotal=round($feetotal+$fee,6);
			file_put_contents('log.txt', "mmmm 5=".$coindaysdestroyedtotal."=".$coindaysdestroyed,FILE_APPEND);
			file_put_contents('log.txt', "\n",FILE_APPEND);
			$coindaysdestroyedtotal=bcadd($coindaysdestroyedtotal,$coindaysdestroyed,9);
			$counttx++;
			
			file_put_contents('log.txt', "mmmm 6",FILE_APPEND);
			file_put_contents('log.txt', "\n",FILE_APPEND);
		}
		
				file_put_contents('log.txt', "ttttt",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);
		...
		file_put_contents('log.txt', "ttttt2",FILE_APPEND);
				file_put_contents('log.txt', "\n",FILE_APPEND);

	}
	
	file_put_contents('log.txt', "aaaaa",FILE_APPEND);
		file_put_contents('log.txt', "\n",FILE_APPEND);

	if ($update_ID!="" || $new_ID!="") {
		file_put_contents('log.txt', "at last new_ID=".$new_ID,FILE_APPEND);
		file_put_contents('log.txt', "\n",FILE_APPEND);
...
//$hash="712227d32a639f1bd8f45b3113f9c7463e5ec685cc372fd14684f0315a425357";
$hash="00009c76b6b741439c93e6d46f7acab2eace5038bc77dc4b946d4b7357e9ed89";

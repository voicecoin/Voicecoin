...
template<typename Stream> void serialize(Stream& s, int8_t a  ) { ser_writedata8(s, a); }
template<typename Stream> void serialize(Stream& s, uint8_t a ) { ser_writedata8(s, a); }
template<typename Stream> void serialize(Stream& s, int16_t a ) { ser_writedata16(s, a); }
template<typename Stream> void serialize(Stream& s, uint16_t a) { ser_writedata16(s, a); }
template<typename Stream> void serialize(Stream& s, int32_t a ) { ser_writedata32(s, a); }
template<typename Stream> void serialize(Stream& s, uint32_t a) { ser_writedata32(s, a); }
template<typename Stream> void serialize(Stream& s, int64_t a ) { ser_writedata64(s, a); }
...
template<typename Stream> void unserialize(Stream& s, int16_t& a ) { a = ser_readdata16(s); }
template<typename Stream> void unserialize(Stream& s, uint16_t& a) { a = ser_readdata16(s); }
template<typename Stream> void unserialize(Stream& s, int32_t& a ) { a = ser_readdata32(s); }
template<typename Stream> void unserialize(Stream& s, uint32_t& a) { a = ser_readdata32(s); }
template<typename Stream> void unserialize(Stream& s, int64_t& a ) { a = ser_readdata64(s); }
...
template<typename I>
unsigned int get_size_of_var_int(I n)
{
    int nRet = 0;
    while (true) {
        nRet++;
...
public:
    ...
    void unserialize(Stream& s)
    {
        size_t size = read_var_int<Stream, uint32_t>(s);
        if (size > Limit) {
...
void unserialize_impl(Stream& is, std::vector<T, A>& v, const unsigned char&)
{
    ...
    while (i < nSize)
    {
        unsigned int blk = _MIN(nSize - i, (unsigned int)(1 + 4999999 / sizeof(T)));
        v.resize(i + blk);
